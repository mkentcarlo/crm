								...
								<div class="col-md-6">
									<table class="table">
										...
										<tr>
											...
											<td>{{ $invoice->additional_fields->passport_no ?? null }}</td>
										</tr>
										<tr>
											<td>Watch Condition:</td>
											<td>{{ $invoice->additional_fields->watch_condition ?? null }}</td>
										</tr>
										<tr>
											<td>Bracelet Condition/Links:</td>
											<td>{{ $invoice->additional_fields->bracelet_condition ?? null }}</td>
										</tr>
										<tr>
											<td>Consignment Period:</td>
											<td>{{ $invoice->additional_fields->consignment_period ?? null }}</td>
										</tr>
										<tr>
											<td>Return Date:</td>
											<td>{{ $invoice->additional_fields->return_date ?? null }}</td>
										</tr>
										<tr>
											<td>Included:</td>
											<td>	
												<div>
													<label>Box:</label> 
													@if($invoice->additional_fields->included && in_array('box', $invoice->additional_fields->included))
												...
												<div>
													...
													@if($invoice->additional_fields->included && in_array('guarantee_card', $invoice->additional_fields->included))
												...
												<div>
													...
													@if($invoice->additional_fields->included && in_array('instructions', $invoice->additional_fields->included))
												...
												<div>
													<label>Others:</label> 
													@if($invoice->additional_fields->included && in_array('others', $invoice->additional_fields->included))
